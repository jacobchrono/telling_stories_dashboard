...
make_widget = pn.widgets.Select(name='Make', options=[''] + df_filtered['make'].unique().tolist(), width=150)
model_widget = pn.widgets.Select(name='Model', options=[''] + df_filtered['model'].unique().tolist(), width=150)
vehicle_type_widget = pn.widgets.Select(name='Vehicle Type', options=[''] + df_filtered['vehicle_type'].unique().tolist(), width=150)
transmission_widget = pn.widgets.Select(name='Transmission', options=[''] + df_filtered['transmission'].unique().tolist(), width=150)
title_widget = pn.widgets.Select(name='Title', options=[''] + df_filtered['title'].unique().tolist(), width=150)
condition_widget = pn.widgets.Select(name='Condition', options=[''] + df_filtered['condition'].unique().tolist(), width=150)
...
header = pn.pane.HTML(
    "<h1 style='font-size:24px; text-align:left;'>Vehicle Dashboard: Focus on Top Makes and Models</h1>"
)
